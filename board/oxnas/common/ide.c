...
static volatile u32 *sata_regs_base[2] =
    { (volatile u32 *)SATA_0_REGS_BASE, (volatile u32 *)SATA_1_REGS_BASE };
static volatile u32 *sata_link_base[2] =
    { (volatile u32 *)SATA_0_LINK_BASE, (volatile u32 *)SATA_1_LINK_BASE };
...
static inline void device_select(int device)
{
	...
	*(sata_regs_base[device] + SATA_DEVICE_SELECT_OFF) =
	    (interface_no << 4) | device_no;
}
...
unsigned char oxnas_sata_inb(int device, int port)
{
	...
	case ATA_PORT_CTL:
		val =
		    (*(sata_regs_base[device] + SATA_ORB4_OFF) &
		     (0xFFUL << SATA_CTL_BIT)) >> SATA_CTL_BIT;
		break;
	case ATA_PORT_FEATURE:
		val =
		    (*(sata_regs_base[device] + SATA_ORB2_OFF) &
	...
	case ATA_PORT_LBAL:
		val =
		    (*(sata_regs_base[device] + SATA_ORB1_OFF) &
...
static inline int wait_sata_command_not_busy(int device)
{
	...
	do {
		if (!
		    (*(sata_regs_base[device] + SATA_COMMAND_OFF) &
		     SATA_CMD_BUSY_BIT)) {
			status = 1;
			break;
		}
...
static unsigned long encode_control_status(oxnas_dma_device_settings_t *
					   src_settings,
					   oxnas_dma_device_settings_t *
					   dst_settings)
{
...
static void dma_start_write(ulong * buffer, int num_bytes)
{
	...
	*DMA_CALC_REG_ADR(SATA_DMA_CHANNEL, DMA_CTRL_STATUS) =
	    encode_control_status(&mem_settings, &oxnas_sata_dma_settings);
	...
	*DMA_CALC_REG_ADR(SATA_DMA_CHANNEL, DMA_BASE_DST_ADR) =
	    oxnas_sata_dma_settings.address_;
	*DMA_CALC_REG_ADR(SATA_DMA_CHANNEL, DMA_BYTE_CNT) =
	    encode_final_eot(&mem_settings, &oxnas_sata_dma_settings,
			     num_bytes);
...
static void wait_dma_xfer(int device)
{
	...
	if (!loops) {
		printf
		    ("wait_dma_xfer() Timed out of wait for SATA core not busy\n");
	} else {
		...
		if (*(sata_regs_base[device] + SATA_INT_STATUS_OFF) &
			...
			unsigned long ctrl_status =
			    *DMA_CALC_REG_ADR(SATA_DMA_CHANNEL,
					      DMA_CTRL_STATUS);
			...
			*DMA_CALC_REG_ADR(SATA_DMA_CHANNEL, DMA_CTRL_STATUS) =
			...
			if (!loops) {
				printf
				    ("wait_dma_xfer() Timed out of wait for DMA channel abort\n");
			}
			...
			*DMA_CALC_REG_ADR(SATA_DMA_CHANNEL, DMA_CTRL_STATUS) =
			...
			if (!loops) {
				printf
				    ("wait_dma_xfer() Timed out of wait for DMA not busy\n");
			}
		}
	}
}
...
static u32 scr_read(int device, unsigned int sc_reg)
{
	...
	*(sata_link_base[device] + OX800SATA_LINK_RD_ADDR) =
...
static void scr_write(int device, unsigned int sc_reg, u32 val)
{
	...
	*(sata_link_base[device] + OX800SATA_LINK_DATA) = val;
	...
	/* Setup adr of required register. std regs start eight into async region */
	*(sata_link_base[device] + OX800SATA_LINK_WR_ADDR) =
	    sc_reg * 4 + SATA_STD_ASYNC_REGS_OFF;
